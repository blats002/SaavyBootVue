...
import { createSaavyRouter } from './router';
import { setAppMenu } from './layout/menuConfig';
...
export function createSaavyApp(config = {}) {
    let combinedRoutes = [...(config.routes || [])];
    let combinedMenu = [...(config.menu || [])];
    let customDashboard = config.dashboard;

    if (config.plugins && Array.isArray(config.plugins)) {
        for (const plugin of config.plugins) {
            if (plugin.routes && Array.isArray(plugin.routes)) {
                combinedRoutes.push(...plugin.routes);
            }
            if (plugin.menu && Array.isArray(plugin.menu)) {
                combinedMenu.push(...plugin.menu);
            }
            if (plugin.dashboard) {
                customDashboard = plugin.dashboard;
            }
        }
    }

    if (combinedMenu.length > 0) {
        setAppMenu(combinedMenu);
    }

    const router = config.router || createSaavyRouter({
